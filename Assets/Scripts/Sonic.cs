...
public class Sonic : MonoBehaviour
{
	...
	void OnTriggerEnter2D(Collider2D collision) {
		...
		{
			GameManager.gm.scope += 1;
			myR.gameObject.GetComponent<AudioSource>().Play(1);
	...
	public void getHit()
	{
		...
		else
		{
			...
			aLoseRings.Play(1);
			GameManager.gm.scope /= 2;
			for (int i = 0; i < GameManager.gm.scope; i++)
			{
	...
	void dead()
	{
		PlayerPrefs.SetInt("deapth", PlayerPrefs.GetInt("deapth") + 1);
		PlayerPrefs.Save();
		aDeath.Play ();
